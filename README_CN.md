...
1. 从 [Releases](https://github.com/Xposed-Modules-Repo/com.eli.mfgx/releases) 下载最新 APK。
2. 安装 APK。
3. 在 LSPosed 中启用模块，勾选 `android` 作用域。
4. 重启设备。
5. 打开应用配置阈值（可选，默认值已可用）。
...
| 上滑 Y 系数 | 1.25 | 虚拟光标移动倍率；>1 放大，<1 缩小 |
| 最远手指距离 | 10000 px | 若某个手指与所有其他手指距离均超过此值则拒绝手势 |
| 三指返回 | 关 | 启用短按返回手势 |
| 返回超时 | 300 ms | 激活后识别短按返回的时间窗口 |
